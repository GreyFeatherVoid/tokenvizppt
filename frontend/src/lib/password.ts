const CLIENT_PASSWORD_SALT = 'tokenvizppt'
...
function toWords(input: Uint8Array): number[] {
  ...
  for (let index = 0; index < input.length; index += 1) {
    words[index >> 2] = (words[index >> 2] ?? 0) | (input[index] << ((index % 4) * 8))
  }
  words[input.length >> 2] = (words[input.length >> 2] ?? 0) | (0x80 << ((input.length % 4) * 8))
  words[(((input.length + 8) >> 6) << 4) + 14] = input.length * 8
  return words
}
...
function md5Hex(value: string): string {
  const words = toWords(new TextEncoder().encode(value))
  let a = 0x67452301
  let b = 0xefcdab89
  let c = 0x98badcfe
  let d = 0x10325476
...
export function clientPasswordDigest(password: string): string {
  return md5Hex(`${password}${CLIENT_PASSWORD_SALT}`)
}
